...
def build_provider_skeleton(*, scene_id: str, beat_ids: list[str], character_ids: list[str], fact_ids: list[str] | None = None, prop_ids: list[str] | None = None, location_ids: list[str] | None = None, runtime_contract: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    beat_ids = [_text(x) for x in (_dict(runtime_contract).get("beat_ids") if isinstance(runtime_contract, dict) else beat_ids)]
    character_ids = [_text(x) for x in (_dict(runtime_contract).get("character_ids") if isinstance(runtime_contract, dict) else character_ids)]
