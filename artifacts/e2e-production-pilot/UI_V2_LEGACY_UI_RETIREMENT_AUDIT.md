...
| `web/src/components/productWorkspaceBatchActions.ts:83,86` | request compatibility | retained for legacy recovery/request shaping; it never grants V2 batch eligibility |
...
|---|---|---|
| `productWorkspaceBatchActions.ts:75-79,310` | request compatibility | V2 snapshot gates production batch readiness; old arrays cannot grant readiness |
...
| `productWorkspaceTaskCenterState.ts:120` | recovery compatibility | missing/unavailable V2 snapshot yields zero production batch eligibility |
| `productWorkspaceTasks.ts:464-465,518-519` | task summary | compatibility summary only |
| `ProductWorkspaceTasksSection.tsx:718` | request fallback | canonical eligibility comes from V2; unavailable V2 stays disabled |
...
The implementation remains review-ready as `PHASE_UI_V2_PRODUCTION_WORKSPACE_READY_FOR_REVIEW`; it is not labelled `UI_V2_COMPLETE`.
