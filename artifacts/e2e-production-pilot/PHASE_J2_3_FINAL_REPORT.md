...
- A PromptIR with no `source_authority` always fails closed. Test fixtures now persist a deterministic Storyboard materialization and PromptIR authority spine, so missing live lineage cannot be inferred from `FRESH` status.
...
- `tests/prompt_ir_authority_fixture.py`: shared deterministic StoryboardShot → MaterializationSet/Pointer → PromptIRVersion/Authority/Pointer builder used by provider-free fixtures.
- `artifacts/e2e-production-pilot/phase_j2_3_fixture_authority_boundary_audit.json`: fixture boundary audit; Production core has zero fixture-marker occurrences.
...
- Full backend final-head rerun on `6e44dbe`: `1760 passed, 4 failed` (1764 collected). The four recorded baseline failures remain outside this change.
...
- Historical `fact_coverage`, `semantic_grounding`, and `semantic_verifier` failures are retained only as historical regression notes and are not counted in this four-node baseline.
- Final targeted rerun after correction: J2.3 and related suites `65 passed`; Phase I pilot included; web `301 passed`.
