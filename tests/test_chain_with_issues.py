"""Real Chain Test with Issues - 真实链路测试（带问题）

创建一个有实际问题的测试场景，验证结构化修复引擎的检测能力。
"""
...
def create_test_story_data_with_issues():
    """创建一个有问题的测试故事数据"""
    return {
        "title": "迷雾寺庙",
        "setting": "古代山间寺庙",
...
def test_chain_with_issues():
    ...
    # 1. 创建测试数据
    story_data = create_test_story_data_with_issues()
    ...
    # 保存报告
    report_path = os.path.join(os.path.dirname(__file__), "..", "artifacts", "chain-test-with-issues-report.json")
