...
def _run_real_authority_pilot(payload: dict, treatment_items: list[dict], blocking_items: list[dict]) -> dict:
    ...
    try:
        ...
        from models import Book, FactSnapshot, Script, ScriptIRVersion, DirectorTreatment, DirectorTreatmentAuthority, DirectorTreatmentPointer, SceneBlocking, SceneBlockingAuthority, SceneBlockingPointer, Session, init_db
        ...
        from core.director_treatment_authority import build_treatment_authority_envelope, payload_hash as treatment_payload_hash, resolve_current_authoritative_treatment, treatment_payload_from_row
        from core.scene_blocking_authority import build_scene_blocking_authority_envelope, blocking_payload_from_row, blocking_payload_hash, resolve_current_authoritative_scene_blocking
        ...
        with Session() as session:
            ...
            for item in treatment_items:
                scene = next(s for s in normalized["scenes"] if s.get("scene_id") == item["treatment"]["scene_id"])
                t = item["treatment"]
                formal = {key: t.get(key, [] if key in {"beat_map", "constraints", "unknowns"} else {} if key == "character_intents" else "") for key in ("scene_id", "scene_name", "dramatic_objective", "audience_question", "character_intents", "beat_map", "relationship_power_shift", "audience_emotion", "information_strategy", "performance_direction", "visual_strategy", "coverage_strategy", "sound_strategy", "edit_rhythm", "constraints", "unknowns", "scene_objective", "dramatic_question", "audience_state_in", "audience_state_out", "suspicion_or_information_strategy", "character_directions", "beat_directions", "director_beat_decisions", "director_contract_version", "performance_arc", "rhythm_strategy", "visual_priority", "scene_exit_intent", "prohibited_interpretations")}
                evidence = {"book_id": book_id, "episode": 1, "scene_id": scene["scene_id"], "scene_name": scene.get("name", ""), "scene": scene, "characters": [], "locked_references": []}
                row = DirectorTreatment(book_id=book_id, episode=1, scene_id=scene["scene_id"], scene_name=scene.get("name", ""), revision=1, status="approved", workflow_profile="production", source_script_revision=str(ir.revision), source_script_hash=ir.payload_hash, source_script_ir_version_id=ir.id, source_script_ir_revision=ir.revision, source_script_ir_hash=ir.payload_hash, source_script_authority_fingerprint=script_ir_envelope["envelope_fingerprint"], source_fact_snapshot_id=str(fact.id), source_fact_snapshot_revision=fact.revision, source_fact_snapshot_hash=fact.payload_hash, dramatic_objective=formal["dramatic_objective"], audience_question=formal["audience_question"], character_intents=json.dumps(formal["character_intents"], ensure_ascii=False), beat_map=json.dumps(formal["beat_map"], ensure_ascii=False), source_constraints=json.dumps({}, ensure_ascii=False), director_decisions=json.dumps(formal, ensure_ascii=False), relationship_power_shift=formal["relationship_power_shift"], audience_emotion=formal["audience_emotion"], information_strategy=formal["information_strategy"], performance_direction=formal["performance_direction"], visual_strategy=formal["visual_strategy"], coverage_strategy=formal["coverage_strategy"], sound_strategy=formal["sound_strategy"], edit_rhythm=formal["edit_rhythm"], constraints=json.dumps(formal["constraints"], ensure_ascii=False), unknowns=json.dumps(formal["unknowns"], ensure_ascii=False), payload_hash=treatment_payload_hash(formal), qualification_state="PRODUCTION_QUALIFIED", stale_status="FRESH", stale_reasons="[]", approved_at=__import__("datetime").datetime.now(), activated_at=__import__("datetime").datetime.now(), created_at=__import__("datetime").datetime.now(), updated_at=__import__("datetime").datetime.now())
                session.add(row); session.flush()
                envelope = build_treatment_authority_envelope(treatment=formal, evidence=evidence, script_ir=normalized, script_ir_version=ir, script_ir_envelope=script_ir_envelope, treatment_id=row.id, treatment_revision=row.revision)
                authority = DirectorTreatmentAuthority(book_id=book_id, episode=1, scene_id=scene["scene_id"], treatment_id=row.id, treatment_revision=row.revision, payload_hash=row.payload_hash, envelope_fingerprint=envelope["envelope_fingerprint"], envelope_json=json.dumps(envelope, ensure_ascii=False, sort_keys=True), qualification_state="PRODUCTION_QUALIFIED", stale_status="FRESH", stale_reasons="[]", approved_at=row.approved_at, activated_at=row.activated_at, created_at=row.created_at, updated_at=row.updated_at)
                session.add(authority); session.flush(); row.authority_envelope_id = authority.id
                pointer = DirectorTreatmentPointer(book_id=book_id, episode=1, scene_id=scene["scene_id"], treatment_id=row.id, treatment_revision=row.revision, authority_envelope_fingerprint=envelope["envelope_fingerprint"], qualification_state="PRODUCTION_QUALIFIED")
                session.add(pointer); session.flush(); session.commit()
                resolved, resolved_env = resolve_current_authoritative_treatment(session, book_id=book_id, episode=1, scene_id=scene["scene_id"])
                treatment_rows.append((row, authority, pointer, resolved_env)); treatment_resolutions.append(resolved)

            blocking_rows, blocking_resolutions = [], []
            for item, treatment_row in zip(blocking_items, treatment_resolutions):
                b = copy.deepcopy(item["blocking"]); b["space"] = b.get("space_model", {})
                for key, default in (("beat_transitions", []), ("spatial_rules", []), ("source_spatial_facts", []), ("creative_decisions", []), ("derived_constraints", {}), ("unresolved_facts", []), ("unknowns", []), ("camera_axis", {}), ("continuity_state", {}), ("asset_authority", {})):
                    b.setdefault(key, default)
                scene = next(s for s in normalized["scenes"] if s.get("scene_id") == b["scene_id"])
                treatment_env = treatment_rows[len(blocking_rows)][3]
                b["source_script_hash"] = ir.payload_hash
                b["treatment_id"] = treatment_row.id
                b["treatment_revision"] = treatment_row.revision
                b["treatment_fingerprint"] = treatment_env["envelope_fingerprint"]
                canonical_model = copy.deepcopy(b)
                row = SceneBlocking(book_id=book_id, episode=1, scene_id=b["scene_id"], scene_name=b["scene_name"], revision=1, status="approved", execution_status="ready", quality_status="qualified", production_status="ready", workflow_profile="production", schema_version="scene_blocking_v2_phase_b", treatment_id=treatment_row.id, treatment_revision=treatment_row.revision, source_script_hash=ir.payload_hash, participants=json.dumps(b.get("participants", []), ensure_ascii=False), beat_transitions=json.dumps([], ensure_ascii=False), spatial_rules=json.dumps([], ensure_ascii=False), unknowns=json.dumps([], ensure_ascii=False), evidence_fingerprint=b.get("blocking_fingerprint", ""), model_info=json.dumps({"mode": "phase_b_real_authority", "provider_calls": 0}, ensure_ascii=False), spatial_model=json.dumps(canonical_model, ensure_ascii=False), source_spatial_facts=json.dumps([], ensure_ascii=False), creative_decisions=json.dumps([], ensure_ascii=False), derived_constraints=json.dumps({}, ensure_ascii=False), unresolved_facts=json.dumps([], ensure_ascii=False), camera_axis=json.dumps({}, ensure_ascii=False), validation=json.dumps(b.get("validation", {}), ensure_ascii=False), source_script_ir_version_id=ir.id, source_script_ir_revision=ir.revision, source_script_ir_hash=ir.payload_hash, source_script_authority_fingerprint=script_ir_envelope["envelope_fingerprint"], treatment_authority_fingerprint=treatment_row.authority_envelope_id and treatment_rows[len(blocking_rows)][3].get("envelope_fingerprint", ""), treatment_payload_hash=treatment_row.payload_hash, source_fact_snapshot_id=str(fact.id), source_fact_snapshot_revision=fact.revision, source_fact_snapshot_hash=fact.payload_hash, source_immutable_raw_hash=raw_hash, qualification_state="PRODUCTION_QUALIFIED", stale_status="FRESH", stale_reasons="[]", activated_at=__import__("datetime").datetime.now(), created_at=__import__("datetime").datetime.now(), updated_at=__import__("datetime").datetime.now())
                session.add(row); session.flush(); row.payload_hash = blocking_payload_hash(blocking_payload_from_row(row)); row.contract_fingerprint = __import__("core.scene_blocking_authority", fromlist=["contract_fingerprint"]).contract_fingerprint(); row.validation_fingerprint = __import__("core.scene_blocking_authority", fromlist=["validation_fingerprint"]).validation_fingerprint(b.get("validation", {})); session.flush()
                envelope = build_scene_blocking_authority_envelope(blocking=b, book_id=book_id, episode=1, scene_id=b["scene_id"], blocking_id=row.id, blocking_revision=row.revision, script_ir_version=ir, script_ir_envelope=script_ir_envelope, treatment=treatment_row, treatment_envelope=treatment_env, fact_snapshot=fact, asset_authority={}, continuity_state={}, validation=b.get("validation", {}))
                authority = SceneBlockingAuthority(book_id=book_id, episode=1, scene_id=b["scene_id"], blocking_id=row.id, blocking_revision=row.revision, payload_hash=row.payload_hash, envelope_fingerprint=envelope["envelope_fingerprint"], envelope_json=json.dumps(envelope, ensure_ascii=False, sort_keys=True), qualification_state="PRODUCTION_QUALIFIED", stale_status="FRESH", stale_reasons="[]", approved_at=row.activated_at, activated_at=row.activated_at, created_at=row.created_at, updated_at=row.updated_at)
                session.add(authority); session.flush(); row.authority_envelope_id = authority.id
                pointer = SceneBlockingPointer(book_id=book_id, episode=1, scene_id=b["scene_id"], blocking_id=row.id, blocking_revision=row.revision, authority_envelope_fingerprint=envelope["envelope_fingerprint"], qualification_state="PRODUCTION_QUALIFIED")
                session.add(pointer); session.commit()
                resolved, resolved_env = resolve_current_authoritative_scene_blocking(session, book_id=book_id, episode=1, scene_id=b["scene_id"])
                blocking_rows.append((row, authority, pointer, resolved_env)); blocking_resolutions.append(resolved)

            return {"database": {"book_id": book_id, "script_ir": {"id": ir.id, "revision": ir.revision, "payload_hash": ir.payload_hash, "authority_envelope_fingerprint": script_ir_envelope["envelope_fingerprint"]}, "fact_snapshot": {"id": fact.id, "revision": fact.revision, "payload_hash": fact.payload_hash}, "script_ir_activation": activation}, "treatment": [{"row_id": r.id, "revision": r.revision, "payload_hash": r.payload_hash, "authority_id": a.id, "authority_fingerprint": a.envelope_fingerprint, "pointer_id": p.id, "pointer_fingerprint": p.authority_envelope_fingerprint, "qualification_state": r.qualification_state} for r, a, p, _ in treatment_rows], "blocking": [{"row_id": r.id, "revision": r.revision, "payload_hash": r.payload_hash, "authority_id": a.id, "authority_fingerprint": a.envelope_fingerprint, "pointer_id": p.id, "pointer_fingerprint": p.authority_envelope_fingerprint, "qualification_state": r.qualification_state, "compiler_version": json.loads(r.spatial_model).get("compiler_version"), "compiled_states_hash": json.loads(r.spatial_model).get("compiled_states_hash")} for r, a, p, _ in blocking_rows], "resolver": {"treatment": [{"scene_id": r.scene_id, "status": "PASS"} for r in treatment_resolutions], "blocking": [{"scene_id": r.scene_id, "status": "PASS"} for r in blocking_resolutions]}}
    finally:
        try:
